...
export async function createSession(req, res) {
  try {
    ...
    // We use clerkId for Stream because the frontend Clerk user ID will match this
...
export async function getActiveSessions(_, res) {
  try {
    const sessions = await Session.find({ status: "active" })
      .populate("host", "name email clerkId") // Added clerkId
      .sort({ createdAt: -1 })
      .limit(20);
...
export async function getMyRecentSessions(req, res) {
  try {
    ...
    const sessions = await Session.find({
      ...
      .populate("host", "name email clerkId") // Added clerkId
      .populate("participant", "name email clerkId") // Added clerkId
      .sort({ createdAt: -1 })
      .limit(20);
...
export async function joinSession(req, res) {
  try {
    ...
    // Compare using MongoDB ObjectIds
